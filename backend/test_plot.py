...
def load_emit(aoi, label):
    ic = (ee.ImageCollection("NASA/EMIT/L2A/RFL")
          .filterBounds(aoi)
...
for scale in (30, 60, 90):
    training  = emit_sel.sampleRegions(training_fc, ["class"], scale=scale, geometries=True)
...
all_plots = (build_plots(1, "coffee", aoi_coffee)
             .merge(build_plots(2, "cocoa", aoi_cocoa)))
...
def add_health(f):
    nv     = ee.Number(ndvi_mosaic.reduceRegion(ee.Reducer.mean(), f.geometry(), 20, maxPixels=1e9).get("NDVI"))
...
with open("test_plots.geojson", "w") as f:
    ...
    print(f"✅ GeoJSON saved to test_plots.geojson ({len(geojson['features'])} features)")